...
from dataengineering.airflow.google_cloud_storage.utils import upload_to_gcs
...
def build_clickhouse_to_gcs_operator(
    local_filename: str,
    gcs_bucket: str,
    gcs_path: str,
    sql: str,
    ...
    class ClickhouseGCSOperator(BaseOperator):
        ...
        def __init__(
            ...
            **kwargs,
            ...
            self.gcs_filename = (
                gcs_filename if gcs_filename is not None else local_filename
            )
...
def build_clickhouse_gcs_to_clickhouse_operator(
    gcs_bucket,
    gcs_path,
    database,
    table,
    ...
    **kwargs,
    ...
    class ClickhouseGCStoCHOperator(BaseOperator):
        ...
        def execute(self, context: Dict[str, Any] = None) -> None:
            ...
            GoogleCloudStorageHook(gcp_conn_id="google_cloud_default").download(
                bucket=self.gcs_bucket,
    ...
    return ClickhouseGCStoCHOperator(
        gcs_bucket,
        gcs_path,
        database,
        table,
...
def build_clickhouse_localfile_to_clickhouse_operator(
    database, table, clickhouse_conn_id, local_filename, file_format, *args, **kwargs
):
    from airflow.models import BaseOperator
    ...
    return ClickhouseLocalFiletoCHOperator(
        database,
        table,
        clickhouse_conn_id,
        local_filename,
...
def build_clickhouse_gcs_folder_to_clickhouse_operator(
    gcs_bucket,
    gcs_path,
    database,
    table,
    ...
    **kwargs,
    ...
    class ClickhouseGCSFoldertoCHOperator(BaseOperator):
        ...
        def execute(self, context: Dict[str, Any] = None) -> None:
            ...
            for file in files:
                filename = (
                    f"{dag_id}_{task_id}_{context.get('ds_nodash')}_{file}".replace(
                        self.gcs_path, ""
                    )
                )
                if os.path.isfile(filename):
                    os.remove(filename)
                gcs_hook.download(
                    bucket=self.gcs_bucket, object=file, filename=filename
                )
    ...
    return ClickhouseGCSFoldertoCHOperator(
        gcs_bucket,
        gcs_path,
        database,
        table,
...
def build_clickhouse_execute_operator(sql, clickhouse_conn_id, *args, **kwargs):
    ...
    return ClickhouseExecuteOperator(sql, clickhouse_conn_id, *args, **kwargs)


def build_clickhouse_bts_streaming_operator():
    from airflow.models import BaseOperator
    ...
    return ClickhouseBTStreamingOperator(
        chain,
        sql,
        log_index,
        sort_columns,
...
def build_ripple_clickhouse_bts_streaming_operator(
    chain, sql, bt_ch_conn_id, ms_ch_conn_id, *args, **kwargs
):
    from airflow.models import BaseOperator
    ...
    return RippleClickhouseBTStreamingOperator(
        chain, sql, bt_ch_conn_id, ms_ch_conn_id, *args, **kwargs
    )


def build_clickhouse_execute_with_uri_operator(
    sql, user, password, uri, *args, **kwargs
):
    from airflow.models import BaseOperator
    ...
    class ClickhouseExecuteWithURIOperator(BaseOperator):
        ...
        def __init__(
            self, sql: str, user: str, password: str, uri: str, *args, **kwargs
        ):
            super().__init__(*args, **kwargs)
            self.sql = sql
            self.user = user
    ...
    return ClickhouseExecuteWithURIOperator(sql, user, password, uri, *args, **kwargs)
