...
def _build_setup_table_operator(
    ...
    if table_type == "staging":
        command = (
            _build_clickhouse_http_command(
                parent_dir=os.path.join(sql_folder, "staging/drop"), resource=resource
            )
            + " && "
        )
...
def _build_table_operator(
    ...
    operator = BashOperator(
        ...
        bash_command=_build_clickhouse_http_command(
            parent_dir=sql_folder, resource=query, clickhouse_uri=clickhouse_uri
        ),
        execution_timeout=timedelta(hours=15),
        env=env,
        dag=dag,
...
def render_sql(sql, sql_variables: dict = None):
    """
...
def execute_query(conn_details: dict, sql: str, sql_variables: dict = None, **kwargs):
    ...
    response_file = (
        kwargs["task_instance"].dag_id + kwargs["task_instance"].task_id + ".txt"
    )
...
def _check_file_for_clickhouse_error(filename):
    ...
    def check_func(check_type):
        ...
        grep_content_e_display = subprocess.run(
            ["grep", "e.displayText()"], stdin=check_type_file.stdout
        )
        grep_content_exception = subprocess.run(
            ["grep", "Exception"], stdin=check_type_file.stdout
        )
        check_type_file.terminate()
        if grep_content_e_display.returncode == 0 or grep_content_exception == 0:
            raise Exception(
                "Clickhouse query unsuccessfull :",
