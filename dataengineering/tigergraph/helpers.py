...
def tigergraph_post_request(tigergraph_request, data, statistic):
    """
    ...
    while max_retries > 0:
        ...
        if response.status_code != 200 or response.json()["error"]:
            ...
            if (
                stats["validLine"] == 0
                or stats["rejectLine"] > 0
                or stats["failedConditionLine"] > 0
                or stats["invalidJson"] > 0
                or stats["oversizeToken"] > 0
                or stats["notEnoughToken"] > 0
                or (stats[statistic][0]["validObject"] - stats["validLine"]) > 1
                or stats[statistic][0]["invalidAttribute"] > 1
            ):
